...
function resolveBookmark(id: string): BookmarkItem | null {
  ...
  if (topic) return { id, title: topic.title, type: 'Topic', link: `/topics/${id}` };

  const question = getQuestionById(id);
  if (question) return { id, title: question.question, type: 'Question', link: `/topics/${question.topicId}` };

  const coding = getCodingProblemById(id);
  if (coding) return { id, title: coding.title, type: 'Coding', link: `/coding/${id}` };

  const mc = getMachineCodingProblemById(id);
  if (mc) return { id, title: mc.title, type: 'Machine Coding', link: `/machine-coding/${id}` };
