...
const SNIPPETS: { name: string; description: string; difficulty: string; category: string; language: 'typescript' | 'javascript' | 'html'; code: string }[] = [
  {
...
export default function Playground() {
  ...
  useEffect(() => {
    ...
    if (sessionSnippet) {
      ...
      if (sessionMode === 'web' || sessionSnippet.includes('<html') || sessionSnippet.includes('<div') || sessionSnippet.includes('<style')) {
        setLanguage('html');
  ...
  const handleLanguageChange = (newLang: 'javascript' | 'typescript' | 'html') => {
    const currentCode = editorRef.current?.getValue() ?? code;
    setLanguage(newLang);

    if (newLang === 'html') {
      setActiveTab('preview');
      if (!currentCode.includes('<div') && !currentCode.includes('<style') && !currentCode.includes('<html')) {
  ...
  const runCode = useCallback((codeToRun?: string) => {
    const rawSource = typeof codeToRun === 'string' ? codeToRun : (editorRef.current?.getValue() ?? code);
    ...
    if (language === 'html' || rawSource.includes('<html') || rawSource.includes('<div') || rawSource.includes('<style')) {
      setActiveTab('preview');
          ...
          injectedContent = `
            ...
            <div class="demo-canvas">
              ${uniqueClasses.length > 0
                ? uniqueClasses.map(cls => `<div class="${cls} demo-card-box"><strong>.${cls}</strong><p style="margin:4px 0 0;font-size:12px;color:#6b6b6b">CSS styles applied</p></div>`).join('')
      ...
      return window.setTimeout(() => {
        if (activeRunIdRef.current === runId) {
          if (typeof handler === 'function') {
            try { handler(...args); } catch (err: any) { appendLine('error', `Async Error: ${err?.message || String(err)}`); }
          }
        }
      }, timeout);
    };

    const sandboxSetInterval = (handler: TimerHandler, timeout?: number, ...args: any[]) => {
      return window.setInterval(() => {
        if (activeRunIdRef.current === runId) {
          if (typeof handler === 'function') {
            try { handler(...args); } catch (err: any) { appendLine('error', `Interval Error: ${err?.message || String(err)}`); }
          }
        }
      }, timeout);
    };
    ...
    try {
      ...
      if (promiseResult && typeof promiseResult.then === 'function') {
        promiseResult
          ...
          .catch((err: unknown) => {
            if (activeRunIdRef.current === runId) {
              appendLine('error', `Runtime Error: ${err instanceof Error ? err.message : String(err)}`);
              setHasError(true);
              setIsRunning(false);
      ...
      appendLine('error', `Syntax / Execution Error: ${err instanceof Error ? err.message : String(err)}`);
      setHasError(true);
      setIsRunning(false);
      setStatusMessage('Error');
      setExecutionTime(performance.now() - startTime);
  ...
  useEffect(() => {
    const handleIframeMessage = (event: MessageEvent) => {
      if (event.data && event.data.type === 'feeq-log') {
        ...
        setOutput(prev => [
          ...prev,
          { id: `${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, type: logType, text, timestamp: Date.now() },
        ]);
      }
    };
  ...
  const loadSnippet = useCallback((snippet: typeof SNIPPETS[number]) => {
    setLanguage(snippet.language);
    setCode(snippet.code);
    tsCacheRef.current = snippet.code;
    if (editorRef.current) {
  ...
  const loadCodingProblem = useCallback((problem: CodingProblem) => {
    if (problem.category === 'CSS' || problem.category === 'HTML & CSS') {
      setLanguage('html');
      setActiveTab('preview');
    } else {
    ...
    const starterCode = problem.implementation.startsWith('<!--')
      ? problem.implementation
      : `// 🎯 ${problem.title} (${problem.difficulty})\n// ${problem.problem}\n\n// Solution implementation:\n${problem.implementation}\n\n// Test invocation:\nconsole.log('Testing solution for ${problem.title}...');\n`;
    setCode(starterCode);
    tsCacheRef.current = starterCode;
    if (editorRef.current) {
      editorRef.current.setValue(starterCode);
  ...
  const difficultyStyle = (difficulty: string) => {
    switch (difficulty) {
      case 'Beginner': return styles.snippetBeginner;
      case 'Intermediate': return styles.snippetIntermediate;
      case 'Advanced': return styles.snippetAdvanced;
      case 'Senior': return styles.snippetSenior;
      default: return styles.snippetBadge;
    }
  };

  const copyConsoleOutput = useCallback(() => {
    const text = output.map(l => `${l.type.toUpperCase()}: ${l.text}`).join('\n');
    ...
    <div className={styles.page}>
      <div className={styles.header}>
        <h1 className={styles.title}>
          <span>🛠️</span> Code Playground & Live Component Sandbox
        </h1>
        <p className={styles.subtitle}>
          Interactive JavaScript, TypeScript, HTML & Modern CSS sandbox with live iframe component previews, streaming console logs, and real-world interview challenges.
        </p>
      </div>

      <div className={`${styles.playgroundContainer} ${isFullscreen ? styles.fullscreenContainer : ''}`}>
        {/* Toolbar */}
        <div className={styles.toolbar}>
          {/* Language Selector */}
          <div className={styles.toolbarGroup}>
            <label className={styles.toolbarLabel} htmlFor="lang-select">Mode / Language:</label>
            <select
              id="lang-select"
              className={styles.select}
              value={language}
              onChange={(e) => handleLanguageChange(e.target.value as 'javascript' | 'typescript' | 'html')}
              aria-label="Language"
            >
          ...
          {/* Theme Selector */}
          <div className={styles.toolbarGroup}>
            <label className={styles.toolbarLabel} htmlFor="theme-select">🎨 Theme:</label>
            <select
              id="theme-select"
              className={styles.select}
              value={selectedTheme}
          ...
          >
            {isRunning ? '⏳ Running...' : language === 'html' ? '▶ Run & Preview' : '▶ Run'}
          </button>
          ...
          <button className={styles.toolbarBtn} onClick={clearOutput} aria-label="Clear output">
            🧹 Clear
          </button>

          <button className={styles.toolbarBtn} onClick={resetCode} aria-label="Reset code">
            ↺ Reset
          </button>
              ...
              <div className={styles.consoleOutput} ref={consoleRef}>
                {output.length === 0 ? (
                  <div className={styles.emptyConsole}>
                    ...
                    <p className={styles.emptyText}>Click <strong>"▶ Run"</strong> or press <strong>⌘+Enter</strong> to execute</p>
                    <span className={styles.emptySub}>Outputs, return values, errors, and async logs stream here</span>
                  </div>
                ) : (
                  output.map((line) => (
                    <div
                      ...
                      className={`${styles.consoleLine} ${
                        ...
                        line.type === 'warn' ? styles.warnLine :
                        line.type === 'info' ? styles.infoLine :
                        line.type === 'result' ? styles.resultLine :
                        styles.logLine
                      }`}
                    >
        ...
        <div className={styles.statusBar}>
          <div className={styles.statusLeft}>
            <span className={hasError ? styles.statusDotError : isRunning ? styles.statusDotRunning : styles.statusDot} />
          ...
          <div className={styles.statusRight}>
            ...
            <span className={styles.themeIndicator}>Theme: {THEMES.find(t => t.id === selectedTheme)?.name}</span>
          </div>
        </div>
      </div>
      ...
      <div className={styles.snippetsSection}>
        <div className={styles.sectionHeader}>
          <h2 className={styles.snippetsTitle}>📚 Practice Snippets (HTML, CSS & JS)</h2>
          <span className={styles.sectionHint}>One-click loads code into editor and executes</span>
        </div>
          ...
          {SNIPPETS.map((snippet) => (
            ...
            >
              <div className={styles.snippetTop}>
                ...
                <span className={difficultyStyle(snippet.difficulty)}>
                  {snippet.difficulty}
                </span>
              </div>
              ...
              <div className={styles.snippetFooter}>
                ...
                <span className={styles.snippetRunHint}>▶ Click to load & run</span>
              </div>
            </button>
          ))}
        </div>
      ...
      {/* Coding Problems from Data */}
      {allCodingProblems.length > 0 && (
        <div className={styles.snippetsSection}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.snippetsTitle}>🧩 Coding Challenge Implementations ({allCodingProblems.length})</h2>
            <span className={styles.sectionHint}>Load algorithm & CSS layout solutions directly into sandbox</span>
          </div>
          <div className={styles.snippetsGrid}>
            {allCodingProblems.slice(0, 16).map((problem) => (
              <button
                key={problem.id}
                className={styles.snippetCard}
                onClick={() => loadCodingProblem(problem)}
              >
                <div className={styles.snippetTop}>
                  <p className={styles.snippetName}>{problem.title}</p>
                  <span className={difficultyStyle(problem.difficulty)}>
                    {problem.difficulty}
                  </span>
                </div>
                <p className={styles.snippetDesc}>{problem.problem.slice(0, 90)}…</p>
                <div className={styles.snippetFooter}>
                  <span className={styles.categoryTag}>{problem.category}</span>
                  <span className={styles.snippetRunHint}>▶ Click to test in editor</span>
                </div>
              </button>
            ))}
          </div>
