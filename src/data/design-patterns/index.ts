...
export const designPatternsTopics: Topic[] = [
  {
    ...
    description: 'Essential design patterns used in frontend development including Singleton, Factory, Module, Observer, Provider, Prototype, and Higher-Order Component patterns with practical JavaScript and React implementations.',
    category: 'Design Patterns',
    difficulty: 'Advanced',
    tags: ['singleton', 'factory', 'module', 'observer', 'provider', 'prototype', 'HOC', 'design-patterns'],
    overview: 'Design patterns are reusable solutions to common software design problems. In frontend development, patterns like Singleton, Factory, Observer, and Provider are foundational to building scalable, maintainable applications. Understanding these patterns helps developers recognize recurring problems, communicate solutions using shared vocabulary, and make informed architectural decisions.',
    concepts: [
...
config.set('apiUrl', 'https://api.example.com');`,
        language: 'typescript',
        explanation: 'Singleton restricts instantiation to one object. Useful for shared configuration, logging, and caching services.'
      },
      {
...
class EventEmitter<Events extends Record<string, unknown>> {
  ...
  emit<K extends keyof Events>(event: K, data: Events[K]): void {
    this.listeners.get(event)?.forEach(listener => listener(data));
        ...
        language: 'typescript',
        explanation: 'Observer pattern enables loose coupling through event-driven communication. Core to frontend event systems, state management, and real-time features.'
      }
    ],
    relatedTopicIds: [],
    questions: [
      {
        id: 'pattern-1',
        question: 'Explain the Singleton pattern. How is it implemented in JavaScript/TypeScript and what are common frontend use cases?',
...
// Cons: Hidden dependencies, hard to test, global state issues`,
        ...
        tags: ['singleton', 'creational-pattern', 'global-state', 'configuration'],
        commonMistakes: [
      ...
      {
        id: 'pattern-2',
        question: 'Explain the Factory pattern. How do you implement a component factory in React?',
...
// Cons: Indirection, harder to trace data flow, type safety requires careful setup`,
        ...
        category: 'Design Patterns',
        topicId: 'frontend-design-patterns',
        tags: ['factory', 'creational-pattern', 'component-factory', 'React', 'dynamic-rendering'],
        commonMistakes: [
      ...
      {
        id: 'pattern-3',
        question: 'Explain the Module pattern. How does it achieve encapsulation in JavaScript and what are its modern equivalents?',
...
function useCounter(initial = 0, max = 100) {
  ...
  return { count, increment, decrement }; // public API
        ...
        category: 'Design Patterns',
        topicId: 'frontend-design-patterns',
        tags: ['module', 'encapsulation', 'IIFE', 'closures', 'private-fields', 'ES-modules'],
        commonMistakes: [
      ...
      {
        id: 'pattern-4',
        question: 'Explain the Observer pattern. How is it used in frontend event systems and state management?',
...
// Cons: Hard to debug event chains, memory leaks if not unsubscribed, ordering issues`,
        ...
        category: 'Design Patterns',
        topicId: 'frontend-design-patterns',
        tags: ['observer', 'behavioral-pattern', 'events', 'pub-sub', 'state-management'],
        commonMistakes: [
      ...
      {
        id: 'pattern-5',
        question: 'Explain the Provider pattern. How does React Context implement it and what are its trade-offs?',
...
function ThemeToggle() {
  ...
  return <button onClick={toggleTheme}>Toggle</button>;
        ...
        category: 'Design Patterns',
        topicId: 'frontend-design-patterns',
        tags: ['provider', 'React-Context', 'prop-drilling', 'composition', 'dependency-injection'],
        commonMistakes: [
      ...
      {
        id: 'pattern-6',
        question: 'Explain the Prototype pattern. How is it used in JavaScript for object creation and cloning?',
...
In frontend applications, the Prototype pattern appears in configuration objects (clone a default config and override specific properties), immutable state management (Redux reducers create new state objects based on previous state), form handling (clone initial form values for reset functionality), undo/redo systems (snapshot state by cloning), and test data factories (clone a template object and override for each test). React's immutability requirement for state updates is essentially the Prototype pattern applied at the state management level — you always create new objects based on previous state rather than mutating in place.`,
        shortAnswer: 'Prototype creates objects by cloning existing instances rather than constructing from scratch. JavaScript is natively prototype-based (prototype chain, Object.create). Practical cloning: spread operator (shallow), structuredClone (deep). Frontend uses: config defaults, immutable state updates, form reset values, undo/redo snapshots, test factories.',
...
// Cons: Deep cloning can be expensive, shared prototype mutations affect all clones`,
        ...
        category: 'Design Patterns',
        topicId: 'frontend-design-patterns',
        tags: ['prototype', 'creational-pattern', 'cloning', 'Object.create', 'immutability'],
        commonMistakes: [
      ...
      {
        id: 'pattern-7',
        question: 'What is the Higher-Order Component (HOC) pattern in React? When should you use it vs. hooks?',
...
// Cons: Wrapper hell, prop collision, ref forwarding, harder to type in TS`,
        ...
        category: 'Design Patterns',
        topicId: 'frontend-design-patterns',
        tags: ['HOC', 'higher-order-component', 'React', 'composition', 'hooks-comparison'],
        commonMistakes: [
      ...
      {
        id: 'pattern-8',
        question: 'Compare all seven design patterns (Singleton, Factory, Module, Observer, Provider, Prototype, HOC). When would you choose each in a frontend project?',
...
The key principle is to match the pattern to the problem, not the other way around. Over-engineering with patterns adds complexity without value. Start with the simplest solution (props, hooks, modules) and introduce patterns when the simple approach creates clear pain points (prop drilling → Provider, conditional rendering boilerplate → Factory, duplicate state management logic → Observer/custom hooks). Patterns are tools in your toolbox, not requirements to check off.`,
      ...
      {
        id: 'pattern-9',
        question: 'How would you implement a real-time notification system using the Observer pattern?',
...
The implementation uses a typed EventEmitter for type safety, a React context/provider for making the notification system available to components, and custom hooks for subscribing to notifications with automatic cleanup. The custom hook pattern ensures that when a component unmounts, its subscription is automatically removed, preventing memory leaks and stale callback execution. For performance, notifications are stored in state using useReducer for predictable updates, and individual notification components use React.memo to prevent unnecessary re-renders when other notifications change.`,
        shortAnswer: 'A notification system uses Observer with three layers: a notification service (WebSocket/API source), a notification manager (subject maintaining observer subscriptions and state), and UI components (observers subscribing via custom hooks). The pattern enables loose coupling, filtered subscriptions, and easy extensibility for new consumers.',
  ...
  return (
    <button aria-label={\`\${unreadCount} unread notifications\`}>
      🔔 {unreadCount > 0 && <span className="badge">{unreadCount}</span>}
        ...
        category: 'Design Patterns',
        topicId: 'frontend-design-patterns',
        tags: ['observer', 'notifications', 'real-time', 'WebSocket', 'architecture'],
        commonMistakes: [
