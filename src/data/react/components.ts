...
export const componentTopics: Topic[] = [
  {
    ...
    description: 'Understanding functional and class components, component composition, reusability, and controlled vs uncontrolled patterns.',
    category: 'React',
    difficulty: 'Beginner',
    tags: ['components', 'functional', 'class', 'composition', 'reusability'],
    overview: 'Components are the building blocks of React applications. They encapsulate UI logic and presentation into reusable, self-contained units. Modern React strongly favors functional components with hooks over class components.',
    concepts: ['Functional components', 'Class components', 'Component composition vs inheritance', 'Controlled vs uncontrolled components', 'Pure components', 'Component reusability patterns'],
    relatedTopicIds: ['react-functional', 'react-class', 'react-props', 'react-hooks'],
    questions: [
      {
        id: 'react-components-1',
        question: 'What is the difference between functional and class components?',
        answer: 'Functional components are plain JavaScript functions that accept a props object and return JSX. With hooks (React 16.8), they can manage state, side effects, context, and all features previously exclusive to class components. They are simpler, produce less boilerplate, and are the recommended approach.\n\nClass components are ES6 classes extending React.Component. They define a render() method returning JSX, manage state through this.state/this.setState, and use lifecycle methods like componentDidMount. They use the this keyword extensively, which introduces complexity around method binding.\n\nA significant difference is how they capture values. Functional components close over props and state from their render — callbacks see values from when they were created. Class components access this.props which always points to the latest values. The React team recommends functional components for all new code; class components are not deprecated but receive no new features.',
        shortAnswer: 'Functional components are plain functions using hooks for state and effects. Class components extend React.Component with lifecycle methods. Functional components are simpler, capture values via closures, and are the modern standard.',
        ...
        commonMistakes: ['Thinking class components are deprecated — they are fully supported, just not recommended for new code.', 'Forgetting to bind this in class component event handlers.', 'Mixing hooks with class components — hooks only work in functional components.'],
        followUps: ['Can hooks completely replace class component features?', 'What is the only feature class components have that hooks cannot replicate?'],
        interviewTips: ['Lead with the modern recommendation (functional + hooks) then explain class components for legacy context.', 'Mention error boundaries as the one remaining class component use case.'],
      },
      {
        id: 'react-components-2',
        question: 'What is component composition and why is it preferred over inheritance?',
        answer: 'Component composition builds complex UIs by combining simpler components, much like composing functions. Instead of class inheritance (Child extends Parent), React encourages components that accept other components as children or props and render them within their own output.\n\nThe primary pattern is the children prop. Content between a component\'s tags is passed as children, allowing components like Card, Modal, or Layout to define structure while remaining content-agnostic. Named slot props (header={<Nav />}, sidebar={<Menu />}) give explicit control over which content appears where.\n\nReact\'s documentation explicitly recommends composition over inheritance — the team has never found a use case where inheritance is necessary. Inheritance creates tight coupling, makes parents fragile, and complicates refactoring. With hooks, even shared logic is better handled through custom hooks than HOCs or inheritance.',
        shortAnswer: 'Component composition builds complex UIs by combining simple components using children and props-as-components patterns. React favors composition because it keeps components independent and avoids the tight coupling of inheritance.',
        ...
        commonMistakes: ['Using inheritance to share behavior — use hooks or composition instead.', 'Overcomplicating with nested slot patterns when children suffices.', 'Confusing the children prop with child component references.'],
        followUps: ['What are render props and how do they relate to composition?', 'How do custom hooks replace HOCs?'],
        interviewTips: ['Quote the React docs: "we haven\'t found any use cases where we would recommend creating component inheritance hierarchies."', 'Show the slots pattern as an alternative to children for multi-region layouts.'],
      },
      {
        ...
        shortAnswer: 'Controlled components have values driven by React state via value + onChange. Uncontrolled components manage their own state in the DOM, read via refs. Controlled is preferred for validation and data control; uncontrolled suits performance-critical or integration scenarios.',
        ...
        commonMistakes: ['Setting value without onChange creates a read-only input.', 'Switching between controlled and uncontrolled by toggling value and defaultValue.', 'Using refs to read controlled input values instead of reading from state.'],
        followUps: ['How does React Hook Form use uncontrolled components for performance?', 'What happens if you set value without an onChange handler?'],
        interviewTips: ['Explain the trade-off: controlled gives power, uncontrolled gives performance.', 'Mention that file inputs are always uncontrolled as a practical edge case.'],
      },
      {
        id: 'react-components-4',
        question: 'What are pure components and how do they optimize rendering?',
        answer: 'A pure component produces the same output for the same props and state — given identical inputs, it always renders identically. This predictability enables React to skip re-renders when nothing has changed, improving performance.\n\nFor class components, React.PureComponent implements shouldComponentUpdate with shallow prop/state comparison. For functional components, React.memo wraps the function and caches results when props are unchanged. Both use reference equality (===) for objects and arrays, so new references bypass the optimization even if values are identical.\n\nPure components are not always appropriate. Cheap components gain little from memoization but pay comparison overhead. Components that almost always receive new props waste time on comparisons. Best candidates are expensive components that render frequently with the same props. Parents must stabilize references with useMemo/useCallback for memo to be effective.',
        shortAnswer: 'Pure components produce identical output for identical inputs. React.PureComponent (class) and React.memo (functional) skip re-renders via shallow comparison. They require stable references for objects/arrays to be effective.',
      ...
      },
      {
        ...
        commonMistakes: ['Making components too specific to one use case.', 'Adding too many props — leads to prop bloat.', 'Not forwarding native HTML attributes.'],
        followUps: ['How do you handle polymorphic components?', 'What is the compound component pattern?'],
        interviewTips: ['Walk through designing a Button step by step to demonstrate your process.', 'Mention TypeScript, testing, and Storybook as supporting tools.'],
      },
    ],
  },
  {
    ...
    questions: [
      {
        ...
        commonMistakes: ['Reading state immediately after calling the setter — state updates on next render.', 'Using direct value form when multiple updates depend on each other.', 'Passing expensive computation without wrapping in a function — it runs every render.'],
        followUps: ['When should you use useReducer instead of useState?', 'How does React 18 batching differ from React 17?'],
        interviewTips: ['Demonstrate batching understanding with multiple setCount calls.', 'Mention the lazy initializer pattern.'],
      },
      {
        id: 'react-state-2',
        question: 'What is state batching in React and how does it work?',
        answer: 'State batching groups multiple state updates into a single re-render. Instead of re-rendering after each setState, React waits until all updates complete and performs one re-render. This dramatically reduces renders and DOM updates.\n\nReact 17 only batched inside React event handlers. Updates in setTimeout, Promises, or native listeners triggered separate re-renders. React 18 introduced automatic batching for all contexts via createRoot. Every setState call in any execution context is batched.\n\nWhen using updater functions in a batch, each updater receives the result of the previous update, ensuring correctness. For rare cases requiring immediate synchronous updates, flushSync from react-dom forces re-render before continuing. Use it sparingly — it bypasses batching optimizations.',
        shortAnswer: 'Batching groups multiple setState calls into one re-render. React 18 batches automatically everywhere (event handlers, promises, timeouts). Updater functions in a batch chain correctly. Use flushSync for rare immediate update needs.',
      ...
      },
      {
        ...
        commonMistakes: ['Mutating state objects directly and expecting re-renders.', 'Forgetting to spread at every nesting level for deep updates.', 'Using array methods that mutate (push, pop, splice, sort).'],
        followUps: ['How does Immer simplify immutable updates?', 'How does immutability enable time-travel debugging?'],
        interviewTips: ['Explain the Object.is check as the reason immutability matters.', 'Mention Immer as a practical tool for complex nested updates.'],
      },
      {
        id: 'react-state-4',
        question: 'When should you use useReducer instead of useState?',
        answer: 'useReducer provides an alternative for complex state logic. It follows the Redux pattern: dispatch actions to a reducer function that computes new state based on current state and action. While useState suffices for simple values, useReducer excels when multiple values change together or transitions are complex.\n\nThe first use case is when multiple state values change together (form with name, email, errors, submission status). A single reducer handles all transitions in one place. The second is complex state machines where transitions depend on both current state and action type (network request states, multi-step wizards).\n\nuseReducer also integrates well with context. Since dispatch is stable (reference never changes), passing it through context avoids unnecessary re-renders in consuming components wrapped with React.memo. The reducer function is pure and testable independently without rendering.',
        shortAnswer: 'Use useReducer when state is complex (multiple related values), transitions depend on current state + action type, you want testable state logic, or when providing update functions through context (dispatch is stable).',
        ...
        commonMistakes: ['Using useReducer for simple toggle/counter state where useState is clearer.', 'Putting side effects in the reducer — reducers must be pure.', 'Forgetting to return new state in every switch case.'],
        followUps: ['How does useReducer compare to Redux?', 'Can you use useReducer with useContext for global state?'],
        interviewTips: ['Explain when to choose useReducer over useState with concrete criteria.', 'Mention that dispatch is a stable reference, ideal for context.'],
      },
      {
        id: 'react-state-5',
        question: 'What is derived state and how should you handle it?',
        answer: 'Derived state is any value computable from existing state or props. A filtered list from items + query is derived state. The principle is: don\'t store what you can compute. Calculate derived values during render — they are always fresh and consistent.\n\nA common anti-pattern is synchronizing state with useEffect. Storing derived values in state and updating via useEffect causes an extra render cycle and adds unnecessary complexity. If you find yourself writing useEffect to "sync" one state with another, the derived value should be computed directly during render instead.\n\nFor expensive computations, use useMemo to cache the result — it recomputes only when dependencies change. Another anti-pattern is copying props into state. If a component receives a prop and stores it in useState, prop changes are ignored because useState initializes only once. Use the prop directly, or use the key prop to reset the component.',
        shortAnswer: 'Derived state is computable from existing state or props — compute it during render instead of storing separately. Use useMemo for expensive computations. Avoid syncing state with useEffect and copying props into state.',
        ...
        commonMistakes: ['Storing computed values in state and syncing with useEffect.', 'Copying props into state, ignoring prop updates.', 'Using useMemo for trivial computations.'],
        followUps: ['How do you decide whether to use useMemo?', 'What is the key-reset pattern?'],
        interviewTips: ['Frame this around "don\'t sync, compute."', 'Mention the React docs article "You Might Not Need an Effect."'],
      },
    ],
  },
  {
    id: 'react-props',
    title: 'Props',
    description: 'Passing data between components via props, destructuring, children, default values, and TypeScript typing.',
    category: 'React',
    difficulty: 'Beginner',
    tags: ['props', 'children', 'destructuring', 'defaults', 'typescript'],
    overview: 'Props are the mechanism for passing data from parent to child components. They are read-only, enabling unidirectional data flow. Understanding how to define, type, and use props effectively is fundamental to composable React applications.',
    concepts: ['Passing and destructuring props', 'The children prop', 'Default prop values', 'TypeScript interfaces', 'Prop drilling', 'Callback props'],
    relatedTopicIds: ['react-components', 'react-data-flow', 'react-context'],
    questions: [
      {
        id: 'react-props-1',
        question: 'What are props and how do they differ from state?',
        answer: 'Props (properties) are inputs passed from parent to child as JSX attributes. They are read-only — a component cannot modify its own props. They flow unidirectionally from parent to child, making the application predictable and debuggable.\n\nState is internal data managed by the component itself, mutable through setState or the useState setter. When state changes, the component and its children re-render. A parent\'s state often becomes a child\'s props — when state changes, new props flow down.\n\nThink of props as function arguments and state as local variables. A function cannot modify its arguments but can create and modify local variables. Similarly, a component cannot modify props but can manage state.',
        shortAnswer: 'Props are read-only inputs from parent to child (like function arguments). State is internal mutable data owned by the component (like local variables). Props flow down unidirectionally.',
        ...
        commonMistakes: ['Trying to modify props inside a child component.', 'Duplicating props into state unnecessarily.', 'Confusing props (from parent) and state (internal).'],
        followUps: ['Can a child communicate back to the parent?', 'What is lifting state up?'],
        interviewTips: ['Use the function argument vs local variable analogy.', 'Mention unidirectional data flow as a design principle.'],
      },
      {
        id: 'react-props-2',
        question: 'What is the children prop and how do you use it?',
        answer: 'The children prop contains content placed between a component\'s opening and closing JSX tags. It allows components to act as wrappers or containers. children can be text, elements, arrays, null, or even functions (render props). The type React.ReactNode covers all cases.\n\nThis enables powerful composition: Layout, Card, Modal define structure while remaining content-agnostic. React.Children utilities (map, forEach, count, toArray) handle edge cases like null children and nested arrays when processing children programmatically.\n\nFor most cases, simply render {children} directly. The render props pattern (passing a function as children) allows parent-to-child data flow where the wrapper provides data to its content, though this pattern has been largely supplanted by hooks.',
        shortAnswer: 'The children prop contains content between a component\'s tags. Typed as React.ReactNode, it enables wrapper/container patterns where components define structure while consumers provide content.',
        ...
        commonMistakes: ['Using React.ReactElement instead of React.ReactNode — ReactNode is broader.', 'Assuming children is always an array.', 'Not using React.Children.map for safe iteration.'],
        followUps: ['What is the difference between React.ReactNode and React.ReactElement?', 'How do you pass data to children?'],
        interviewTips: ['Show the composition value: children makes components reusable containers.', 'Mention React.Children utilities for edge cases.'],
      },
      {
        id: 'react-props-3',
        question: 'What is prop drilling and how do you avoid it?',
        answer: 'Prop drilling passes data through intermediate components that don\'t use it, just to reach a deeply nested consumer. If App has user data that a deeply nested Avatar needs, the data passes through Header → NavBar → UserSection → Avatar, even though intermediates never use it.\n\nThis increases coupling, makes refactoring harder (adding/removing a prop requires changes in every intermediate), and obscures data flow. Three solutions exist: Context API broadcasts data to any descendant without passing through intermediates. Component composition restructures the hierarchy so parents render nested components directly. State management libraries (Redux, Zustand) provide centralized stores.\n\nComposition should be tried first — it is simpler and keeps data flow explicit. Context suits widely-shared data (auth, theme, locale). State management libraries suit complex cross-cutting state. Don\'t overuse avoidance patterns for shallow trees where direct passing is clearer.',
        shortAnswer: 'Prop drilling passes data through intermediate components that don\'t use it. Avoid with Context API (widely-shared data), component composition (restructure hierarchy), or state management libraries (complex global state).',
        ...
        commonMistakes: ['Reaching for context before trying composition.', 'Using a single context for all app data.', 'Overusing avoidance patterns for shallow trees.'],
        followUps: ['How does Context cause performance issues?', 'When should you use a state management library vs Context?'],
        interviewTips: ['Show multiple solutions and explain when each is appropriate.', 'Emphasize composition first — it is simpler.'],
      },
      {
        id: 'react-props-4',
        question: 'How do you set default prop values in React?',
        answer: 'The recommended approach for functional components is JavaScript default parameters in the destructuring pattern. This is idiomatic, works with TypeScript, and keeps defaults close to usage. Defaults trigger on undefined (not null).\n\nFor complex defaults like objects or arrays, define the default outside the component to avoid creating new references every render. An inline default ({}) creates a new object each render, which defeats memoization if passed to a child wrapped in React.memo.\n\nTypeScript enhances defaults by marking props as optional (?) while providing defaults in destructuring. Class components use static defaultProps, which still works but is de-emphasized in modern React.',
        shortAnswer: 'Use JavaScript default parameters in destructuring: function Button({ size = "md" }). Define complex defaults outside the component for stable references. Defaults trigger for undefined, not null.',
        ...
        commonMistakes: ['Defining object/array defaults inline — creates new references every render.', 'Confusing undefined and null — defaults only apply for undefined.', 'Using defaultProps with functional components.'],
        followUps: ['How does null vs undefined affect defaults?', 'Why should complex defaults be outside the component?'],
        interviewTips: ['Mention reference stability for object defaults.', 'Explain the null vs undefined distinction.'],
      },
      {
        id: 'react-props-5',
        question: 'How do you type props effectively with TypeScript?',
        answer: 'Type props with interfaces or type aliases. For components wrapping native HTML elements, extend built-in attribute types (React.ButtonHTMLAttributes<HTMLButtonElement>) to accept both custom and native attributes. Use the rest/spread pattern to forward native props.\n\nDiscriminated unions model mutually exclusive configurations: { as: "button"; onClick: () => void } | { as: "link"; href: string }. TypeScript narrows based on the discriminant. Generic components accept type parameters flowing through props for end-to-end type safety.\n\nAvoid React.FC — it implicitly included children (fixed in React 18 types), doesn\'t support generics cleanly, and obscures the return type. Prefer typing the props parameter directly.',
        shortAnswer: 'Type props with interfaces. Extend native HTML attributes for wrapper components. Use discriminated unions for mutually exclusive configs and generics for type-safe data components. Avoid React.FC.',
        ...
        commonMistakes: ['Using any for props.', 'Not extending native HTML attributes for wrapper components.', 'Using React.FC which doesn\'t support generics cleanly.'],
        followUps: ['How do you type the ref prop with forwardRef?', 'What are Omit, Pick, and Partial useful for in props?'],
        interviewTips: ['Show a real-world generic example.', 'Explain why React.FC is discouraged.'],
      },
    ],
  },
];
