...
export const customHookTopics: Topic[] = [
  {
    ...
    title: 'Custom Hooks, Rules of Hooks, Stale Closures & Advanced Hook Patterns',
    description:
    ...
    tags: ['react', 'hooks', 'custom-hooks', 'useReducer', 'stale-closures', 'useCallback', 'useMemo', 'useRef'],
    overview:
    ...
    questions: [
      {
        id: 'react-hooks-1',
        question: 'Why must Hooks only be called at the top level of a component (never inside loops, conditions, or nested functions)?',
        answer:
...
function GoodComponent({ isSpecial }: { isSpecial: boolean }) {
  ...
  return <div>{count}</div>;
        ...
        followUps: ['How does React 19 useActionState or use() hook relax some conditional constraints for Promises/Context?'],
        interviewTips: ['Mention the `fiber.memoizedState` linked list to explain the exact internal reason.']
      },
      {
        id: 'react-hooks-2',
        question: 'What is the "Stale Closure" problem in React Hooks, and what are the three ways to fix it?',
        answer:
...
export function Timer() {
  ...
  return <div>Count: {count}</div>;
        ...
        category: 'React',
        topicId: 'react-custom-hooks-rules',
        tags: ['stale-closures', 'closures', 'useEffect', 'useRef', 'functional-updates'],
        commonMistakes: [
        ...
        followUps: ['How does the useEvent (or useEffectEvent) experimental hook solve the closure problem for callbacks?'],
        interviewTips: ['Clearly demonstrate the functional update `setX(prev => ...)` pattern as the first line of defense.']
      },
      {
        id: 'react-hooks-3',
        question: 'How do you design and structure production-grade Custom Hooks in React? Build a useDebounce and useLocalStorage custom hook.',
        answer:
...
export function useLocalStorage<T>(key: string, initialValue: T) {
  ...
  return [storedValue, setValue] as const;
        ...
        followUps: ['How do you synchronize useLocalStorage across multiple browser tabs using the window "storage" event?'],
        interviewTips: ['Add the window "storage" event listener to useLocalStorage to impress interviewers.']
      }
    ]
  }
];
