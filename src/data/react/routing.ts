...
export const routingTopics: Topic[] = [
  {
    ...
    tags: ['react', 'routing', 'react-router', 'spa', 'nested-routes', 'loaders', 'actions'],
    overview:
    ...
    questions: [
      {
        id: 'react-routing-1',
        question: 'How does client-side routing work under the hood using the HTML5 History API, and how do React Router nested routes with <Outlet /> work?',
        answer:
...
export const router = createBrowserRouter([
  {
    ...
    children: [
      { index: true, element: <div>Dashboard Overview</div> },
      {
        ...
        followUps: ['Why is Nginx try_files $uri /index.html required when deploying SPAs to production?'],
        interviewTips: ['Mention web server fallback configuration to show real-world production deployment experience.']
      },
      {
        id: 'react-routing-2',
        question: 'What are React Router Loaders and Actions, and how do they eliminate "fetch-on-render" waterfalls?',
        answer:
  ...
  return (
    <div>
      ...
      <Form method="post">
        ...
        category: 'React',
        topicId: 'react-routing',
        tags: ['loaders', 'actions', 'data-fetching', 'waterfalls', 'react-router'],
        commonMistakes: [
          'Mixing useEffect data fetching with route loaders, losing parallel prefetching benefits.',
        ],
        followUps: ['How does Remix / React Router v7 leverage loaders and actions for SSR?'],
        interviewTips: ['Contrast "Fetch-on-Render" (useEffect waterfall) with "Render-as-You-Fetch" (Loaders / Suspense).']
      }
    ]
  }
];
