  ...
  examples: [
    {
      ...
      explanation:
        "Top-level snake_case keys are converted to camelCase. Primitive values are unchanged.",
    },
    {
      input: `camelCaseKeys({\n  user_profile: {\n    first_name: 'Bob',\n    address_info: { street_name: '123 Main' }\n  }\n})`,
      output: `{ userProfile: { firstName: 'Bob', addressInfo: { streetName: '123 Main' } } }`,
      explanation:
        "Nested object keys are recursively converted at all levels.",
    },
    {
      input: `camelCaseKeys([\n  { item_id: 1, item_name: 'Apple' },\n  { item_id: 2, item_name: 'Banana' },\n])`,
      output: `[{ itemId: 1, itemName: 'Apple' }, { itemId: 2, itemName: 'Banana' }]`,
      explanation:
        "Array elements that are objects have their keys converted; the array structure is preserved.",
    },
  ],
...
console.log(camelCaseKeys({ already_camel: 'ok' })); // { alreadyCamel: 'ok' }`,
  ...
  timeComplexity:
    "O(n * k) where n is the total number of key-value pairs across all levels and k is the average key length.",
  spaceComplexity:
    "O(n) for the new transformed object (full copy), plus O(d) recursion stack depth for nesting depth d.",
