  ...
  examples: [
    {
      ...
      output: "[10, 20, 30]",
      explanation:
        "Resolves with all three values in order once the asynchronous p3 finishes.",
    },
    {
...
5. In the \`.catch(err)\` callback:
  ...
  timeComplexity:
    "O(N) where N is the number of promises to attach handlers, with concurrent execution time equal to the slowest individual promise (max(T_i)).",
