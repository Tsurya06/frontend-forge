  ...
  examples: [
    {
      ...
      explanation:
        "Two h2s nest under the first h1. The second h1 starts a new top-level section.",
    },
    {
      ...
      explanation:
        "Skipped level (h1 -> h3) still nests under h1. The h2 also nests under h1 since it has a higher level number.",
    },
    {
      input: `[]`,
      output: `[]`,
...
function buildTableOfContents(headings: Heading[]): TocNode[] {
  ...
  timeComplexity:
    "O(n) — each heading is pushed and popped from the stack at most once.",
  spaceComplexity:
    "O(n) for the output tree nodes, plus O(d) for the stack where d is the maximum heading depth (at most 6).",
