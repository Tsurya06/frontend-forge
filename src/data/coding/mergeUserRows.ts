...
Your solution should be efficient (single pass using a Map for O(n) performance), handle missing or null fields gracefully, and produce clean output without duplicate array entries when specified.`,
  ...
  { userId: 2, name: "Bob", roles: ["user"] }
      ...
      explanation:
        "Alice's rows are merged: name from first row, email from third, roles concatenated.",
    },
    {
      input: `mergeUserRows([
  { userId: 1, score: 10 },
  ...
  { userId: 1, score: 15 }
      ...
      explanation:
        "Scalar values use last-write-wins: the final score of 15 overwrites 10 and 25.",
    },
    {
      input: `mergeUserRows([
  ...
  { userId: "a", tags: ["ts", "js"], level: 2 }
      ...
      explanation:
        'With deduplication enabled, duplicate "js" in tags is removed.',
    },
  ],
...
function mergeUserRows(rows: Row[], options: MergeOptions = {}): Row[] {
  ...
  timeComplexity:
    "O(n * k) where n is the number of rows and k is the average number of properties per row.",
  spaceComplexity:
    "O(u * k) where u is the number of unique users and k is the total number of unique properties.",
