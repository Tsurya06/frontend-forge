...
export const packageManagementTopics: Topic[] = [
  {
    ...
    description: 'Understanding npm, Yarn, package.json configuration, lock files, semantic versioning, and dependency management strategies for JavaScript projects.',
    category: 'Package Management',
    difficulty: 'Intermediate',
    tags: ['npm', 'yarn', 'package.json', 'semver', 'dependencies', 'lock-files'],
    overview: 'Package managers are essential tools for managing third-party dependencies in JavaScript projects. Understanding npm and Yarn, package.json configuration, lock files, semantic versioning, and the different types of dependencies helps developers manage projects effectively, avoid version conflicts, and maintain secure, reproducible builds.',
    concepts: [
  ...
  "devDependencies": {
    ...
    "eslint": "^9.6.0"
        ...
        language: 'json',
        explanation: 'dependencies are needed at runtime; devDependencies are only needed during development/build. The ^ prefix allows minor version updates.'
      }
    ],
    relatedTopicIds: [],
    questions: [
      {
        id: 'pkg-1',
        question: 'What is semantic versioning (semver) and how do version ranges work in package.json?',
...
Understanding version ranges is crucial for maintaining stable applications. Using ^ (the default) is generally recommended because it receives bug fixes and new features automatically while protecting against breaking changes. However, not all packages follow semver strictly — some introduce breaking changes in minor versions, and some bugs are introduced in patch versions. This is why lock files exist: they record the exact versions that were installed, ensuring every developer and CI environment uses identical versions regardless of what version range the package.json specifies. The package.json defines the acceptable range; the lock file records the specific resolution within that range.`,
        shortAnswer: 'Semver uses MAJOR.MINOR.PATCH: major = breaking changes, minor = new features (backwards-compatible), patch = bug fixes. Version ranges in package.json: ^ (caret) allows minor+patch updates, ~ (tilde) allows only patch updates, exact version locks to a specific version. Lock files record the exact resolved versions for reproducibility.',
...
// npm audit             — check for vulnerabilities`,
      ...
      {
        id: 'pkg-2',
        question: 'What is the difference between package-lock.json and yarn.lock? Why are lock files important?',
...
git commit -m "chore: update dependencies"`,
        ...
        category: 'Package Management',
        topicId: 'package-management-fundamentals',
        tags: ['lock-files', 'package-lock.json', 'yarn.lock', 'npm-ci', 'reproducibility'],
        commonMistakes: [
...
NODE_ENV=production npm ci           # same effect`,
        ...
        category: 'Package Management',
        topicId: 'package-management-fundamentals',
        tags: ['dependencies', 'devDependencies', 'peerDependencies', 'npm', 'package.json'],
        commonMistakes: [
      ...
      {
        id: 'pkg-4',
        question: 'Compare npm and Yarn. What are their differences and what about pnpm?',
...
# package.json: "packageManager": "pnpm@9.4.0"`,
      ...
      {
        id: 'pkg-5',
        question: 'How do you handle dependency conflicts and version mismatches in a project?',
...
Prevention is better than cure. Keep dependencies up to date with regular updates (use Dependabot or Renovate for automated PRs). Before adding a dependency, check its dependency tree (\`npm info package dependencies\`) for potential conflicts. Minimize the number of dependencies — each one increases the chance of version conflicts. Use \`npm outdated\` regularly to identify packages that are behind on updates, and address them before they become incompatible with the rest of the dependency tree.`,
        shortAnswer: 'Dependency conflicts occur when packages require incompatible versions of a shared dependency. Diagnose with npm ls, resolve with npm dedupe (compatible ranges), npm overrides / yarn resolutions (force versions), or updating packages. Prevent by keeping dependencies current, minimizing them, and using Dependabot/Renovate for automated updates.',
...
#     open-pull-requests-limit: 10`,
        ...
        category: 'Package Management',
        topicId: 'package-management-fundamentals',
        tags: ['dependency-conflicts', 'npm-overrides', 'resolutions', 'dedupe', 'version-management'],
        commonMistakes: [
      ...
      {
        id: 'pkg-6',
        question: 'What are npm scripts and how do you use them effectively in a project?',
...
// Cross-platform compatibility
{
  "scripts": {
    ...
    "env": "cross-env NODE_ENV=production node build.js"
        ...
        category: 'Package Management',
        topicId: 'package-management-fundamentals',
        tags: ['npm-scripts', 'package.json', 'automation', 'build-scripts', 'lifecycle'],
        commonMistakes: [
