...
export const buildToolsTopics: Topic[] = [
  {
    ...
    description: 'Understanding Webpack\'s module bundling system including loaders, plugins, code splitting, tree shaking, and build optimization.',
    category: 'Build Tools',
    difficulty: 'Advanced',
    tags: ['webpack', 'bundling', 'loaders', 'plugins', 'code-splitting', 'tree-shaking'],
    overview: 'Webpack is the most widely used module bundler for JavaScript applications. It processes your application\'s dependency graph starting from one or more entry points, combining all modules into optimized bundles. Understanding Webpack\'s core concepts — entry, output, loaders, plugins, and optimization — is essential for configuring builds, debugging build issues, and optimizing application performance.',
    concepts: [
    ...
    questions: [
      {
        id: 'build-1',
        question: 'What are Webpack loaders and plugins? How do they differ and what are common examples of each?',
...
The distinction is scope and timing. Loaders work on individual files during module processing — they answer "how do I transform this .scss file into something Webpack can understand?" Plugins work on the entire build — they answer "what do I want to do with the compiled output?" A practical example: css-loader (loader) resolves CSS imports and url() references into modules. MiniCssExtractPlugin (plugin) takes all those CSS modules and combines them into separate .css files in the output. Both are needed for production CSS handling, but they operate at different phases of the build.`,
        shortAnswer: 'Loaders transform individual files during module resolution (e.g., babel-loader converts JSX to JS, css-loader resolves CSS imports). Plugins hook into the build lifecycle for broader tasks (e.g., HtmlWebpackPlugin generates HTML, MiniCssExtractPlugin extracts CSS files). Loaders handle per-file transformation; plugins handle compilation-wide operations.',
...
module.exports = {
  ...
  plugins: [
    ...
    new BundleAnalyzerPlugin({
      ...
      {
        id: 'build-2',
        question: 'How does Webpack code splitting work? Explain entry points, dynamic imports, and the SplitChunksPlugin.',
...
Webpack 5 introduced additional optimizations: module federation for sharing code between independently built applications, improved tree shaking with nested tree shaking and inner module tree shaking, and persistent caching that stores build results on disk to speed up subsequent builds.`,
        shortAnswer: 'Webpack splits bundles via: entry points (multiple entries = multiple bundles), dynamic imports (import() creates async chunks), and SplitChunksPlugin (extracts shared/vendor code automatically). Dynamic imports with React.lazy enable route-based splitting. SplitChunksPlugin separates vendor code for independent caching. Contenthash filenames ensure cache busting on changes.',
...
const HeroImage = import(
  /* webpackPreload: true */ './components/HeroImage'
        ...
        category: 'Build Tools',
        topicId: 'webpack-bundling',
        tags: ['code-splitting', 'webpack', 'chunks', 'dynamic-import', 'SplitChunksPlugin'],
        commonMistakes: [
      ...
      {
        id: 'build-3',
        question: 'How does tree shaking work in Webpack? What configuration is needed to enable it?',
...
// Using MathUtils.add includes multiply too — single default export unit`,
        ...
        category: 'Build Tools',
        topicId: 'webpack-bundling',
        tags: ['tree-shaking', 'webpack', 'dead-code-elimination', 'ES-modules', 'optimization'],
        commonMistakes: [
      ...
      {
        id: 'build-4',
        question: 'Compare Webpack, Vite, and esbuild. What are the key differences and when would you choose each?',
...
The modern recommendation for most frontend projects is Vite. It provides the best developer experience (instant startup, fast HMR), reasonable production builds (via Rollup), excellent framework integration, and minimal configuration. Use Webpack for existing projects with complex configurations or when you need its extensive plugin ecosystem. Use esbuild for library builds, scripts, or situations where raw build speed is the priority.`,
        shortAnswer: 'Webpack: mature, highly configurable, plugin-rich but slow. Vite: native ESM in dev (instant startup), Rollup for production, best DX for new projects. esbuild: Go-based, 10-100x faster, great for libraries but limited plugin ecosystem. Choose Vite for new apps, Webpack for complex existing projects, esbuild for library builds and speed-critical pipelines.',
        difficulty: 'Advanced',
        type: 'Conceptual',
        category: 'Build Tools',
        topicId: 'webpack-bundling',
        tags: ['webpack', 'vite', 'esbuild', 'bundler-comparison', 'build-tools'],
        commonMistakes: [
...
export default defineConfig({
  ...
  server: {
    hmr: {
      overlay: true, // show error overlay
        ...
        category: 'Build Tools',
        topicId: 'webpack-bundling',
        tags: ['HMR', 'hot-module-replacement', 'development', 'React-Fast-Refresh', 'DX'],
        commonMistakes: [
  ...
  },
  {
    ...
    description: 'Understanding Babel\'s role in JavaScript transpilation including presets, plugins, polyfills, and browser compatibility strategies.',
    category: 'Build Tools',
    difficulty: 'Intermediate',
    tags: ['babel', 'transpilation', 'presets', 'plugins', 'polyfills', 'browser-compatibility'],
    overview: 'Babel is a JavaScript compiler that transforms modern JavaScript and JSX syntax into backwards-compatible versions that older browsers can execute. Understanding Babel\'s preset and plugin system, its relationship with browserslist, and how polyfills complement transpilation is essential for building applications that work across all target browsers.',
    concepts: [
    ...
    questions: [
      {
        id: 'build-6',
        question: 'What is Babel and why is it needed? Explain presets, plugins, and the relationship with browserslist.',
...
Browserslist is a configuration format that specifies target browser/environment versions, shared across tools (Babel, PostCSS/Autoprefixer, ESLint). You define targets like "> 0.5%, last 2 versions, not dead" in package.json or a .browserslistrc file. @babel/preset-env reads this configuration and applies only the transformations needed for browsers that don't support a given feature. This keeps bundle size minimal — there's no point transforming async/await for a target list where all browsers support it natively. The key distinction is that Babel handles syntax transformations (arrow functions → regular functions) but not built-in additions (Promise, Map, Array.prototype.includes). For those, you need polyfills, which @babel/preset-env can automatically inject based on usage with the useBuiltIns: 'usage' option.`,
        shortAnswer: 'Babel transpiles modern JavaScript to backwards-compatible code using plugins (individual transformations) and presets (plugin collections). @babel/preset-env uses browserslist to apply only necessary transformations for target browsers. Babel handles syntax transformation; polyfills (via core-js) add missing runtime features. @babel/preset-react handles JSX.',
...
// package.json browserslist (alternative location)
{
  "browserslist": [
    ...
    "not dead"
      ...
      {
        id: 'build-7',
        question: 'What is the difference between transpilation and polyfilling? How does @babel/preset-env handle both?',
...
// Runtime: regenerator-runtime polyfill to execute generators`,
        ...
        category: 'Build Tools',
        topicId: 'babel-transpilation',
        tags: ['transpilation', 'polyfills', 'core-js', 'babel', 'browser-compatibility'],
        commonMistakes: [
  ...
  },
  {
    ...
    questions: [
      {
        id: 'build-8',
        question: 'How do you configure ESLint for a TypeScript React project? Explain rules, plugins, and extends.',
...
The extends property lets you use pre-built configurations that enable recommended rule sets. Extending eslint:recommended enables core ESLint rules that catch common issues. plugin:@typescript-eslint/recommended enables essential TypeScript rules. plugin:react/recommended enables React best practices. Teams often layer these: start with recommended configs, then customize individual rules based on team preferences. The flat config format simplifies this by treating everything as arrays of configuration objects that are merged in order, with later configs overriding earlier ones. Integration with Prettier is handled by eslint-config-prettier, which disables all ESLint rules that conflict with Prettier's formatting, letting each tool focus on what it does best.`,
        shortAnswer: 'Configure ESLint for TypeScript React with: @typescript-eslint/parser (TS parsing), @typescript-eslint/eslint-plugin (TS rules), eslint-plugin-react and react-hooks (React rules), eslint-plugin-jsx-a11y (accessibility). Use extends for recommended rule sets, customize individual rules, and add eslint-config-prettier to avoid formatting conflicts.',
...
// package.json scripts
{
  "scripts": {
    ...
    "lint:fix": "eslint src/ --fix"
      ...
      {
        id: 'build-9',
        question: 'How do ESLint and Prettier work together? Why do you need both and how do you prevent conflicts?',
...
// 5. CI → prettier --check + eslint --max-warnings 0`,
        ...
        category: 'Build Tools',
        topicId: 'eslint-linting',
        tags: ['eslint', 'prettier', 'formatting', 'lint-staged', 'husky', 'pre-commit'],
        commonMistakes: [
