...
const pool = new WorkerPool('worker.js', navigator.hardwareConcurrency || 4);`,
        ...
        category: "JavaScript",
        topicId: "js-concurrency",
        tags: ["Web Workers", "Limitations", "Worker Pool", "DOM", "Memory"],
        commonMistakes: [
...
self.addEventListener('message', (event) => {
  if (event.data.type === 'GET_VERSION') {
    event.source.postMessage({ type: 'VERSION', version: CACHE_NAME });
        ...
        category: "JavaScript",
        topicId: "js-concurrency",
        tags: ["Service Workers", "PWA", "Caching", "Offline", "Lifecycle"],
        commonMistakes: [
  ...
  await Promise.all(
    pending.map(async (entry) => {
      ...
      await db.delete('outbox', entry.id);
        ...
        relatedTopics: ["Cache API", "Workbox", "Background Sync", "IndexedDB"],
      },
      {
        id: "js-conc-6",
        question:
